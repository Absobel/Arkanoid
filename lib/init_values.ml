...
module BriquesInit = struct
  ...
  let br_list =
    let create_brick x y  =
      (x, y), Graphics.rgb (Random.int 255) (Random.int 255) (Random.int 255) in
    let rec fill acc x y max_x max_y=
      if x > max_x then
        fill acc 0. (y +. br_height) max_x max_y
      else if y > max_y then
        acc
      else
        fill ((create_brick x y) :: acc) (x +. br_width) y max_x max_y in
